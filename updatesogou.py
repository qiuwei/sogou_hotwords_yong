...
def sendmessage(message, note):
    subprocess.Popen(['notify-send', message, note])
    return

def dldict(dl_name):
    from urllib2 import Request, urlopen, URLError, HTTPError
    
    url ='http://download.pinyin.sogou.com/dict/download_cell.php?id=4&name=%CD%F8%C2%E7%C1%F7%D0%D0%D0%C2%B4%CA%A1%BE%B9%D9%B7%BD%CD%C6%BC%F6%A1%BF'
    req = Request(url)
    req.add_header('User-Agent', 'Mozilla/5.0 (X11; Linux x86_64; rv:16.0) Gecko/20100101 Firefox/16.0')
    req.add_header('Referer', 'http://pinyin.sogou.com')

    try:
        f = urlopen(req)
        print "Downloading " + url

        localfile = open(dl_name, "wb")
...
if __name__ == '__main__':
    ...
    message = re.sub(r'\s+', '',examples.rstrip('\0'))
    message = re.sub(u'\u3000', '', message)
    #message = re.sub(u'(\uff09)([^$])', u'\g<1>、\g<2>', message)
    message = re.sub(u'(\uff09)(?=[^$])', u'\g<1>、' , message)
    #print formed_msg
    sendmessage("网络词库已更新:", message.encode('utf8'))
    os.system('ibus-daemon -rdx')
